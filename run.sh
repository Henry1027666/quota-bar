...
#
# 为什么用 pkill+nohup 而不是 LaunchAgent：本机曾注册 LaunchAgent
# com.henryzhang.quotabar（登录自启 + KeepAlive），但 launchd 启动这个
# 裸 adhoc 签名可执行文件会被系统代码签名策略拦截（Launch Constraint
# Violation / OS_REASON_CODESIGNING），KeepAlive 反复拉起失败导致 runs 疯涨
# 且与脚本启动叠加出双实例。故已移除该 LaunchAgent，统一用本脚本
# pkill 旧实例 → nohup 新实例，保证恒为单实例。
set -euo pipefail
